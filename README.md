...
### Lecture 36: 8 May 2024

Brief discussion of application of out-of-plane transmission to the design of optical "metasurfaces" to shape free-space light by large aperiodic nanopatterned surfaces typically resembling photonic-crystal slabs but with unit cells that vary with position.  A typical way to analyze light transmission through such structures is the locally periodic approximation (LPA), in which transmission through each unit cell is computed as if it were periodic, and the resulting approximate transmitted fields are then propagated to the far field with the principle of equivalence.  An analogous method is the locally *uniform* approximation (LUA), more commonly known as scalar diffraction theory or local ray optics, in which a surface (slowly varying on the λ scale) is modeled locally as a uniform/flat surface; this approach has been widely applied in diffractive optical devices such as Fresnel lenses and binary phase gratings.
